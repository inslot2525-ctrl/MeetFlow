...
const [liveText, setLiveText] = useState("");
  ...
  const handleClear = () => {
    ...
    editor?.commands.insertContent(`<h1>Product Sync</h1><p style="color:#9ca3af"><em>Live transcript below — cleared fresh for testing.</em></p>`);
  };
    ...
    <DarkGradientBg className="flex h-screen text-white overflow-hidden font-sans">
      ...
      <div className="w-[380px] shrink-0 flex flex-col border-r border-white/10 p-5 gap-3 overflow-hidden">
        ...
        <div className="flex gap-2">
          <button onClick={handleClear} className="flex-1 bg-white/10 border border-white/20 text-white py-2 rounded-full text-xs font-semibold hover:bg-white/20">Clear — fresh start</button>
          <button onClick={simulateLiveMeeting} className="flex-1 bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 py-2 rounded-full text-xs font-semibold hover:bg-cyan-500/25">Simulate Sample</button>
        </div>
